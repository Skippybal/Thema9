...
In table 1 shows the five number summary of all of the numerical columns that are present in the dataset. Inspecting this table closely a deviation in the mean and the median of most of the columns can be observed. This is an indication that our data might be skewed. This is an important observation, seeing as some algorithms expect the data to have a Gaussian distribution. Any type of normalization or transformation was dealt with in the data cleaning. The NA's that are present will also be removed in the data cleaning.
...
In figure 3 the area per lipid is again plotted against the bending rigidity, only this time, the plot is colored based on the saturation index.
...
title <- expression(atop(bold("Figure 3:"), 
                         scriptstyle("APL against bending rigidity")))
annotate_figure(plots,
                top=text_grob(title))
```

The results shown in figure 3 show an interesting pattern. It seems that a specific saturation index is only found in membranes within a specific range of area per lipid and bending rigidity.
...
All of the results from the data exploraition led to the following reseach question: Is it possible to use machine learning to reliably predicts the sterol type in a membrane with a higher than 80% accuracy, given the area per lipid, the bending rigidity and the compressibility?


## Data cleaning

To make the dataset usable for machine learning it had to be cleaned up using various of methods. The difference between the cleaned dataset and the original dataset can be seen when comparing table 2 and table 3. Unused columns like temperature and ethanol concentration where removed. All of the data in the columns is scaled using min max scaling. This is visible in table 3, seeing as the values now range from zero to one. NA's where also omitted. It was decided not to remove any variables based on a high degree of correlation. 
...
## Discussion
### Exploration
The PCA plot was made before any of the data cleaning step. This means that the data wasn't transformed or normalized. The function used to plot the PCA plot did have the option to scale the data enabled, which should have taken care of any scaling issues. Except that this function might make use of z score scaling, in which case the results might not be totally accurate seeing as there is a slight skew in some of our variables. Z score scaling assumes that our data has a normal distribution, which is why it might have caused problems.
