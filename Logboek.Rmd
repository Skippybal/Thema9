...
```{r}
chart <- ggplot(data, aes(sterol.conc, thickness, colour = factor(sterol.type), shape = factor(tails) )) + 
         geom_point(alpha=1/10) + geom_jitter()
chart
```
```{r}
chart2 <- ggplot(data, aes(bending, satur.index, colour = factor(sterol.type) )) + 
        geom_jitter(alpha=1)
chart2
```
```{r}
