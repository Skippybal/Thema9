...
Here is a function to put a specific column as the last column for the encoding in the .arff file. Some of the code is commented out so it won't create a lot of datasets that take up memory.
```{r MakeLast}
library(RWeka)
...
#Mutate_df(nona_copy, "sterol.conc")
...
\begin{table}[thb]
\caption{\label{Sterol_accuracy}Sterol prediction accuracy}
\footnotesize
...
\begin{table}[thb]
\caption{\label{Sterol_keys}Sterol prediction accuracy (Key)}
\scriptsize
{\centering
...
\begin{table}[thb]
\caption{\label{Tails_Accuracy}Accuracy tails prediction}
\footnotesize
{\centering \begin{tabular}{lrr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}c}
\\
\hline
...
\begin{table}[thb]
\caption{\label{Keys}Accuracy tails prediction (Key)}
\scriptsize
{\centering
\begin{tabularx}{\textwidth}{lX}\\
(1) & rules.ZeroR '' 48055541465867954 \\
...
The results can be found in table 12, the keys/commands for every algorithm can be found in table 13. It looks like RandomForest and LMT score very high. Lets set RandomForest as our base now to compare which is significantly better.
...
\begin{table}[thb]
\caption{\label{RF_Tails}Tails prediction, RandomForest as base of t test}
\footnotesize
{\centering \begin{tabular}{lrr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}c}
\\
\hline
...
\begin{table}[thb]
\caption{\label{RF_Tails_keys}Tails prediction, RandomForest base (Key)}
\scriptsize
{\centering
\begin{tabularx}{\textwidth}{lX}\\
(1) & rules.ZeroR '' 48055541465867954 \\
...
\begin{table}[thb]
\caption{\label{Ens_tails}Ensemble learning tails prediction accuracy}
\footnotesize
...
\begin{table}[thb]
\caption{\label{Ens_tails_keys}Ensemble learning tail prediction (Key)}
\scriptsize
{\centering
\begin{tabularx}{\textwidth}{lX}\\
...
Here we look at the area under the curve for all of the algoritms to check for a difference, the results are shown in table 16. It uses the same algorithms/keys as in table 15, also shown in table 17 for convenience.
...
\begin{table}[thb]
\caption{\label{sterol_con_pred}Sterol concentration prediction accuracy}
\footnotesize
{\centering \begin{tabular}{lrr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}c}
\\
\hline
...
\begin{table}[thb]
\caption{\label{sterol_con_pred_key}Sterol concentration prediction accuracy (Key)}
\scriptsize
{\centering
\begin{tabularx}{\textwidth}{lX}\\
(1) & rules.ZeroR '' 48055541465867954 \\
...
Looking at table 18, it seems like the LMT tree gives significantly better results then any of the other models. Let's take a look whether or not we can get some sort of improvements with ensemble learning. Here I will use the LMT tree, seeing as I now have a better understanding of it due to some papers. However, due to time restraints, I am unable to redo all of the experiments for the tails prediction and make a new model.
...
\begin{table}[thb]
\caption{\label{Ens_Ste_conc}Ensemble learning sterol concentration prediction accuracy}
\footnotesize
{\centering \begin{tabular}{lrr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}cr@{\hspace{0.1cm}}c}
\\
\hline
...
\begin{table}[thb]
\caption{\label{Ens_Ste_conc_keys}Ensemble learning sterol concentration prediction accuracy (Key)}
\scriptsize
{\centering
\begin{tabularx}{\textwidth}{lX}\\
