...
# Data Cleaning
Here is a quick code snippet to show us the skewdness of the data. This will be usefull when desideng whether or not to use standardization or normalization on the columns.
```{r Skew}
apply(data[c("temperature","sterol.conc","satur.index",
                      "PC.conc","ethanol.conc","APL", "thickness", "bending",
...
```{r SkewHist}
hist.data.frame(data[c("temperature","sterol.conc","satur.index",
                      "PC.conc","ethanol.conc","APL", "thickness", "bending",
                      "tilt", "zorder","compress")])
```
...
Seeing as most of the distributions aren't Gaussian we should use min_max normalization(scaling). This does lessen/supress the impact of outliers. We could also use z score normalization (standardization), but this assumes that our distributions are Gaussian, which they are not. 
...
Here is a PCA plot that is made using the data after applying log and cube transformations to reduce skewdness.
```{r PCA2}
# Create PCA plot
new_help <- log10(correlation[2:6])
new_help$APL <- corrolation$APL^2
